 ...
 * This eliminates the need for an external server. This class does not support loopback connections.
 *
 * 직접 TCP 연결을 신호 채널로 사용하는 AppRTCClient 를 구현합니다.
 * 이렇게 하면 외부 서버가 필요하지 않습니다. 이 클래스는 루프백 연결을 지원하지 않습니다.
 */

/*
DirectRTCClient (필수)

- TCP Socket 을 통해 P2P 영상통화를 하기 위한 클래스.

- ConnectActivity 에서, EditText 창에 상대방의 IP를 적으면 CallActivity 에서 판별하여 객체로 생성되는 클래스.

- 보통 WebSocket 을 사용하여 appr.tc와 연결되므로 이 클래스는 잘 사용하지 않음.

- 그러나, 쓸 일이 있을 수 있으므로 삭제하지 않음.

- TCPChannelClient 와 같이 씀
 */
